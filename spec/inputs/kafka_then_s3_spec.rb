...
class TestKafkaGZGroup < Kafka::Group
  ...
  describe LogStash::Inputs::KafkaThenS3 do
    ...
    let (:kafkaThenS3_config) {{'topic_id' => 'test',
                          'bucket' => 'test_bucket',
                          "access_key_id" => "1234",
    ...
    let (:bad_kafkaThenS3_config) {{'topic_id' => 'test',
                              'white_list' => 'other_topic',
                              'bucket' => 'test_bucket'
    }}
    let (:white_list_kafkaThenS3_config) {{'white_list' => 'other_topic','bucket' => 'test_bucket'}}
    ...
    it "should register" do
      ...
      input = LogStash::Plugin.lookup("input", "kafka_then_s3").new(kafkaThenS3_config)
      expect {input.register}.to_not raise_error
      FileUtils.rm_rf(temporary_directory)
    end

    it "should register with whitelist" do
      input = LogStash::Plugin.lookup("input", "kafka_then_s3").new(white_list_kafkaThenS3_config)
      expect {input.register}.to_not raise_error
    end

    it "should fail with multiple topic configs" do
      input = LogStash::Plugin.lookup("input", "kafka_then_s3").new(bad_kafkaThenS3_config)
    ...
    it 'should populate kafkaThenS3 config with default values' do
      kafkaThenS3 = LogStash::Inputs::TestKafkaThenS3.new(kafkaThenS3_config)
      insist {kafkaThenS3.zk_connect} == 'localhost:2181'
      insist {kafkaThenS3.topic_id} == 'test'
      insist {kafkaThenS3.group_id} == 'logstash'
      !insist {kafkaThenS3.reset_beginning }
    end
    ...
    it 'should retrieve event from kafka and fetch a plain text file from S3' do
      ...
      kafkaThenS3 = LogStash::Inputs::TestKafkaThenS3.new(kafkaThenS3_config)
      expect(kafkaThenS3).to receive(:create_consumer_group) do |options|
        TestKafkaGroup.new(options)
      end
      kafkaThenS3.register
